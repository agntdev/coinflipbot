# fix-7f65a70c28caa554 — `/help` does not list `/flip` or `/flip3` — core commands are undiscoverable

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CFB

The spec says `/help` should list all available commands with one-line descriptions. The actual help text in `src/handlers/E2T2.ts:7` and `src/handlers/T01.ts:7` only lists `/start` and `/help` — the bot's core flip commands (`/flip`, `/flip3`) are absent. Users cannot discover the main functionality of CoinFlipBot through the help system.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-7f65a70c28caa554.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-7f65a70c28caa554.json` (and any new command `tests/commands/fix-7f65a70c28caa554.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-7f65a70c28caa554.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-7f65a70c28caa554.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
