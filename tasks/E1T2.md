...
Implement the /flip3 command handler in src/handlers/flip3.ts. The handler should generate three random results using Node's crypto.randomInt, then return them joined by ', '. Add a test spec to tests/specs/flip3.json and register the command in tests/commands.json.
