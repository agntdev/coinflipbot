# fix-5b3e06cc2a3f718a — Environment variable name mismatch: BOT_TOKEN vs TELEGRAM_BOT_TOKEN

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CFB

`docs/spec.md` specifies the bot token env var as `TELEGRAM_BOT_TOKEN` (lines 17, 62, 83), but `src/index.ts:4` reads `process.env.BOT_TOKEN` and `.env.example` has `BOT_TOKEN=`. If an operator follows the spec and sets `TELEGRAM_BOT_TOKEN`, the bot will fail at startup with `BOT_TOKEN is required`. The spec and the runtime are contractually inconsistent — one of them is wrong.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-5b3e06cc2a3f718a.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-5b3e06cc2a3f718a.json` (and any new command `tests/commands/fix-5b3e06cc2a3f718a.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-5b3e06cc2a3f718a.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-5b3e06cc2a3f718a.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
