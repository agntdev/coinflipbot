# fix-35fa47234e8638c4 — Duplicate /start command declaration in tests/commands

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CFB

Both `tests/commands/E2T1.json` and `tests/commands/start.json` declare `["/start"]`. The tests-gate globs and merges all `tests/commands/*.json` files. While a de-duplication step may silently merge them, duplicate declarations create ambiguity about which file owns the command registration and could mask errors if the merge logic changes. One should be removed.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-35fa47234e8638c4.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-35fa47234e8638c4.json` (and any new command `tests/commands/fix-35fa47234e8638c4.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-35fa47234e8638c4.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-35fa47234e8638c4.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
