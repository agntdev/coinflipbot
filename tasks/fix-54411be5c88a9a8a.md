# fix-54411be5c88a9a8a — Duplicate /help handlers cause double replies at runtime

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CFB

Both `src/handlers/E2T2.ts` (`:1`) and `src/handlers/help.ts` (`:1`) implement `/help` with identical replies. Since `buildBot()` auto-loads all handler files, both composers fire on `/help`, producing two identical help messages. Same root cause as the `/start` duplication.

Remove one of the two handler files.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-54411be5c88a9a8a.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-54411be5c88a9a8a.json` (and any new command `tests/commands/fix-54411be5c88a9a8a.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-54411be5c88a9a8a.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-54411be5c88a9a8a.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
