...
Implement the /flip command handler in src/handlers/flip.ts. The handler should use Node's crypto.randomInt to generate a random 0 or 1, then return 'Heads' or 'Tails' accordingly. Add a test spec to tests/specs/flip.json and register the command in tests/commands.json.
