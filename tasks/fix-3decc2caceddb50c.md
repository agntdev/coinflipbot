# fix-3decc2caceddb50c — Duplicate /start handlers cause double replies at runtime

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CFB

Both `src/handlers/E2T1.ts` (`:1`) and `src/handlers/start.ts` (`:1`) implement `/start` handlers. `buildBot()` auto-loads every `.ts` file in `src/handlers/` (`src/bot.ts:39`), so both composers fire on `/start`. The user receives **two** welcome messages: first the E2T1 plain-text reply, then the start.ts reply with the inline keyboard. This is a runtime defect.

Either delete the duplicate file (prefer keeping the younger/idiomatic `composer.command()` implementation in `start.ts`) or merge them into one handler.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-3decc2caceddb50c.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-3decc2caceddb50c.json` (and any new command `tests/commands/fix-3decc2caceddb50c.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-3decc2caceddb50c.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-3decc2caceddb50c.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
