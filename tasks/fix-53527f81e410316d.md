# fix-53527f81e410316d — Duplicate `/help` command handler — T01 is dead code shadowed by E2T2

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CFB

`src/handlers/E2T2.ts` and `src/handlers/T01.ts` are identical files: both define a composer with `composer.command("help", ...)` returning the same text. Due to alphabetical loading order in `src/bot.ts:39` (`files.sort()`), E2T2 is loaded before T01, so E2T2's `/help` handler always wins and T01's is dead code. Both also declare `["/help"]` in their respective `tests/commands/` manifests, creating redundant coverage claims.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-53527f81e410316d.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-53527f81e410316d.json` (and any new command `tests/commands/fix-53527f81e410316d.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-53527f81e410316d.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-53527f81e410316d.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
